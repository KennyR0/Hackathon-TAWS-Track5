...
def reset_backend_singletons(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.delenv("TWELVE_DATA_API_KEY_2", raising=False)
    monkeypatch.delenv("COINGECKO_API_KEY", raising=False)
    monkeypatch.delenv("COINGECKO_API_KEY_2", raising=False)
    monkeypatch.delenv("FRED_API_KEY", raising=False)
    monkeypatch.delenv("FRED_API_KEY_2", raising=False)
    monkeypatch.delenv("EIA_API_KEY", raising=False)
    monkeypatch.delenv("MARKET_PROVIDER_BUDGETS", raising=False)
    monkeypatch.delenv("RAPIDAPI_KEY", raising=False)
    monkeypatch.delenv("YAHOO_FINANCE_API_HOST", raising=False)
    monkeypatch.delenv("YAHOO_FINANCE_BASE_URL", raising=False)
    monkeypatch.delenv("YAHOO_FINANCE_HISTORY_PATH", raising=False)
