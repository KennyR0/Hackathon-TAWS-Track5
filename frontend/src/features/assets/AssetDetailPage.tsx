...
export function AssetDetailPage() {
  ...
  if (eventsQuery.isLoading || signalsQuery.isLoading) return <LoadingSkeleton rows={10} />
  if (!asset) {
    return (
      <EmptyState
  ...
  return (
    <div className="page-stack">
      ...
      {asset.latestSignal ? <ReactionChart signal={asset.latestSignal} /> : null}

      <SurfaceCard eyebrow="Limitaciones honestas" title="Cobertura del endpoint actual">
        <ul className="text-list">
          <li>No hay endpoint dedicado de series profundas por activo.</li>
